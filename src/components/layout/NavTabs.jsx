            ...
            {TABS.map((tab) => {
                ...
                return (
                    <Link
                        ...
                        className={`relative px-4 py-2.5 text-sm font-semibold hover:no-underline transition-colors ${
                            active ? 'text-text-primary' : 'text-text-secondary hover:text-text-primary'
                        }`}
                    >
                        {t(`nav.tabs.${tab.key}`)}
                        {active && <span aria-hidden="true" className="absolute inset-x-4 -bottom-px h-0.5 bg-gold" />}
                    </Link>
                );
            })}
        </div>
