...
// Every face occupies the same grid cell, so the button is as wide as the widest of them whatever
// it is currently showing. `whitespace-nowrap` because a face that wraps inside a fixed cell is
// the height jumping instead of the width.
const faceClass = 'col-start-1 row-start-1 flex items-center gap-2 whitespace-nowrap';
...
function SubscribeButton({ channelId, variant = 'inline', className = '' }) {
    ...
    return (
        <button
            ...
            className={`group flex items-center justify-center px-5 py-2.5 rounded-full font-semibold text-sm
        ...
        >
            {/* No "in flight" face. The state shown IS the state pressed for -- the mutation
                ...
                All three faces are rendered, stacked in ONE grid cell, with the two that do not
                apply hidden. That is what makes the button a fixed width: «اشتراك», «مشترك» and
                «إلغاء الاشتراك» are three quite different lengths, so a button sized by whichever
                is showing changed width on the press AND again on every hover -- shoving the
                channel banner's other controls sideways each time, which is the jitter the
                in-flight face was already removed to avoid. `invisible` rather than `hidden`:
                the cell has to keep being as wide as the widest of them, and visibility:hidden
            ...
            <span className="grid place-items-center">
                ...
                <span className={`${faceClass} ${
                    ...
                    <Check size={18} /> {t('channel.subscribed')}
                </span>
                {/* The warning, reached two ways: hover/focus on a subscribed button, or the
                    armed state that stands in for hover where there is no pointer to hover with.
                    Armed shows it outright -- there is nothing to swap back to until the press
                    lands or the window closes. */}
                <span className={`${faceClass} ${
                    !subscribed
                        ? 'invisible'
                        : showArmed
