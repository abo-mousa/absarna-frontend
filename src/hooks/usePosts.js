 ...
 */
export const usePostsFeed = (followed, size = 20) => {
    const scope = useUserScope();
    return useInfiniteQuery({
        queryKey: followed ? ['posts', { followed: true, size }, scope] : ['posts', { followed: false, size }],
        queryFn: async ({ pageParam = 0 }) => {
            const res = await api.get('/posts', { params: { page: pageParam, size, followed: followed || undefined } });
            return res.data;
        },
        initialPageParam: 0,
